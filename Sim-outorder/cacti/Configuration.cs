...
namespace Simoutorder
{
	...
	{
		public int CacheSize { get { return TwoPowered (lg2CacheSize); } }
		public int Sets { get { return TwoPowered(lg2Sets); } }
		public int LineSize { get { return TwoPowered(lg2LineSize); } }

		private int TwoPowered(int n)
		{
			var pow = Math.Pow (2, n);
			return (int)pow;
		}

		public void WriteToCactiFile (string file)
		{
			// $@"for verbatim {interpolated} string"
			var lines = new string[] {
				$@"# Cache size",
				$@"-size (bytes) {CacheSize}",
				$@"",
				$@"# Line size",
				$@"-block size (bytes) {LineSize}",
				$@"",
